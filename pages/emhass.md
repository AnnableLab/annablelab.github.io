      ...
      payload:
        ...
        load_cost_forecast: |-
          {% set ns = namespace(
            ...
            output={
              now().isoformat(): states(sensors.amber.general_price) | float(0)
            }
          ) %}
        ...
        prod_price_forecast: |-
          {% set ns = namespace(
            ...
            output={
              now().isoformat(): states(sensors.amber.feed_in_price) | float(0)
            }
          ) %}
          {% for day in range(num_prediction_days) %}
            {% for forecast in ns.input %}
        ...
        pv_power_forecast: |-
          {% set ns = namespace(
            ...
            output={
              now().isoformat(): (states(sensors.solar.current_power) | float(0) * 1000) | round
            }
          ) %}
